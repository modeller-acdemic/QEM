# -*- coding: utf-8 -*-
"""
Created on Fri Aug 29 18:09:27 2025

This code simply trains the random forest regressor using the feature-engineered data from the collector
It also verifies the accuracy of the feature engineering by computing the L2 error from the raw data, too
...
def transformed_error(results_df):
    ...
    # initialise a list to collect the l2 distances per circuit
    transformed_errors = []
    ...
    for _, group in results_df.groupby('circuit_index'):
        ...
        # compute the l2 norm between the vectors
        l2_distance = np.linalg.norm(ideal_vector - noisy_vector)
        
        # append the distance as a float to the output list
        transformed_errors.append(float(l2_distance))
...
def mitigation_error(results_dataframe, feature_matrix, fitted_model):
    ...
    for circuit_index, circuit_group in results_with_predictions.groupby('circuit_index'):
        ...
        # compute the l2 norm between the ideal and predicted vectors
        l2_distance_per_circuit = np.linalg.norm(ideal_vector_per_circuit - predicted_vector_per_circuit)
        per_circuit_mitigation_error_errors.append(float(l2_distance_per_circuit))
...
def raw_error(raw_df):       
    ...
    for _, row in raw_df.sort_values('circuit_index').iterrows():
        ...
        # compute the l2 distance between the ideal and noisy vectors
        l2_distance = np.linalg.norm(np.array(qiskit_ideal_vector) - np.array(qiskit_noisy_vector))
        
        # append the distance to the output list
        raw_circuit_errors.append(float(l2_distance))
...
def hyperparameter_tuning(results_df, X, y, fraction, random_state=42, param_dicts=None, estimator_class=None):
    ...
    for design_index, params in enumerate(param_dicts, start=1):
        ...
        # run the external test pipeline and read back the generalised mitigated l2 metric
...
def main(fraction=1.0):
    print(f"Running model training with {int(fraction*100)}% simulated data...")
    
    # input data
    results_path = rf'C:\Users\Chris\Dropbox\Uni\MSc AI\11 IRP\Code\Phase 1 Circuit Definitions\simulation_results_{int(fraction*100)}.csv'
    raw_path = rf'C:\Users\Chris\Dropbox\Uni\MSc AI\11 IRP\Code\Phase 1 Circuit Definitions\raw_outputs_{int(fraction*100)}.csv'
    
    # output model name
    model_filename = f"models/model_{int(fraction*100)}_RFRetuned.joblib"
    ...
    # compute l2 errors from the transformed data
    transformed_circuit_errors = transformed_error(results_df)
    
    # compute and print the average transformed l2 error
    average_l2_error_transformed = float(np.mean(np.asarray(transformed_circuit_errors, dtype=float)))
    print(f"Average L2 error from transformed data: {average_l2_error_transformed}")
    
    # compute l2 errors verified through qiskit from raw counts
    raw_circuit_errors = raw_error(raw_df)
    
    # compute and print the average raw l2 error
    average_l2_error_raw = float(np.mean(np.asarray(raw_circuit_errors, dtype=float)))
    print(f"Average L2 error from raw outputs: {average_l2_error_raw}")
    ...
    # the random forest model with the tuned hyperparameters
    ...
    # models for robustness test - following tuning
    ...
    # compute mitigation l2 error using the trained model across the full dataset
    average_mitigation_error = mitigation_error(results_df, X, model)
    print(f"Average L2 mitigation error: {average_mitigation_error}")
