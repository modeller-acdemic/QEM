...
def main(fraction, model):
    ...
    # input paths
    model_path = f"../Data/models/model_{int(fraction*100)}{model}.joblib"
    unmitigated_path = "../Data/test_data/test_data_raw.csv"
    training_results_path = f"../Data/training_data/feature_engineered/engineered_results_{int(fraction*100)}.csv"
    ...
    mitigated_output_path = f"../Data/mitigated_results/test_data_mitigated_{int(fraction*100)}{model}.csv"
...
if __name__ == "__main__":
    ...
    # call main for all models under original test data
    models = ["", "_DT", "_GBR", "_RFRetuned"]
    for model in models:
        for fraction_value in [0.0, 0.25, 0.50, 0.75, 1.0]:
            main(fraction=fraction_value, model=model)
    
    # call for retest vals (manually change input data and output to include _rt suffix)
    # for fraction_value in [0.0, 0.25, 0.50, 0.75, 1.0]:
